...
import com.datamantra.logs_analyzer.producer.KafkaProducerHandler
import com.datamantra.producer.KafkaAvroProducerHandler
import com.datamantra.utils.{ApacheAccessLogCombined, ApacheAccessLog}
 ...
 * @author ${user.name}
 */
object ApacheLogGeneratorAvroKafka {
  ...
  def main(args : Array[String]) {
    ...
    val n_records_per_day = args(0).toLong
    val kafkaBrokerList = args(1)
    val kafkaTopicName = args(2)
    var kafkaAvroHandler: KafkaAvroProducerHandler = null

    kafkaAvroHandler = new KafkaAvroProducerHandler(kafkaBrokerList, kafkaTopicName)
    val logDataFeedActor = new ApacheLogGeneratorAvroKafka
    logDataFeedActor.initialize
    logDataFeedActor.LogGenerator(d, n_records_per_day, kafkaAvroHandler)
  }

}


class ApacheLogGeneratorAvroKafka {

  val str: String = "Hello, How are you"
  private val ipA_by_ctry = Array.ofDim[Int](ApacheLogGeneratorAvroKafka.MAX_IPS_PER_COUNTRY,ApacheLogGeneratorAvroKafka.N_COUNTRIES)
  private val ipB_by_ctry = Array.ofDim[Int](ApacheLogGeneratorAvroKafka.MAX_IPS_PER_COUNTRY,ApacheLogGeneratorAvroKafka.N_COUNTRIES)
  private val tot_ips_by_ctry: Array[Int] = new Array[Int](ApacheLogGeneratorAvroKafka.N_COUNTRIES)
  private val cum_hourly_weight_by_ctry = Array.ofDim[Int](24,ApacheLogGeneratorAvroKafka.N_COUNTRIES)
  ...
  private val requests: Array[String] = new Array[String](ApacheLogGeneratorAvroKafka.MAX_FILE_LINES)
  private val referrers: Array[String] = new Array[String](ApacheLogGeneratorAvroKafka.MAX_FILE_LINES)
  private val user_agents: Array[String] = new Array[String](ApacheLogGeneratorAvroKafka.MAX_FILE_LINES)
  ...
  def initialize: Unit = {
    ...
    for (i <- 0 to ApacheLogGeneratorAvroKafka.N_COUNTRIES - 1) {
      tot_ips_by_ctry(i) = 0
      ctry_ind.put(ctry_abbr(i), String.valueOf(i))
    }

    for (i <- 0 to ApacheLogGeneratorAvroKafka.MAX_IPS_PER_COUNTRY - 1) {
      for (j <- 0 to ApacheLogGeneratorAvroKafka.N_COUNTRIES - 1) {
        ipA_by_ctry(i)(j) = 0
        ipB_by_ctry(i)(j) = 0
      }
    }
    ...
    for (line <- Source.fromInputStream(getClass.getResourceAsStream("/all_classbs.txt")).getLines()) {
      ...
      if (fields.length == 2) {
        ...
        if (i_ctry_s != null) {
          ...
          if (tot_ips_by_ctry(i_ctry) < ApacheLogGeneratorAvroKafka.MAX_IPS_PER_COUNTRY) {
  ...
  def weight: Unit= {
    ...
    var hourly_weight_by_ctry = Array.ofDim[Int](24,ApacheLogGeneratorAvroKafka.N_COUNTRIES)


    for( hour <- 0 to 23){
      for(ctry <- 0 to ApacheLogGeneratorAvroKafka.N_COUNTRIES - 1){
  ...
  def LogGenerator(d: Date, n_records_per_day: Long, kafkaAvroHandler: KafkaAvroProducerHandler): Unit = {
    ...
    //Write to Socket
    //val socket = new Socket(InetAddress.getByName("localhost"), 44444)
    ...
    var count = 0
         while(true) {
            ...
            clicks_left = 1 + rand.nextInt(ApacheLogGeneratorAvroKafka.MAX_CLICKS_PER_USER)
            ...
            kafkaAvroHandler.publishAvroRecord(apacheLogEvent)
           println(output)
          // out.println(output)
           /*
            buffer += output
            ...
            if(count == 10) {
              kafkaHandler.publishBuffered(buffer)
              count = 0
            }
           */

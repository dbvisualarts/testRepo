package com.datamantra.logs_analyzer.loggenerator

import java.io.{PrintStream, PrintWriter, DataOutputStream, FileOutputStream}
...
object ApacheLogGeneratorKafka {
  ...
  def main(args : Array[String]) {
    ...
    var kafkaHandler: KafkaProducerHandler = null

    kafkaHandler = new KafkaProducerHandler(kafkaBrokerList, kafkaTopicName)
    val logDataFeedActor = new ApacheLogGeneratorKafka
    logDataFeedActor.initialize
    logDataFeedActor.LogGenerator(d, n_records_per_day, kafkaHandler)
  }
...
class ApacheLogGeneratorKafka {
  ...
  def LogGenerator(d: Date, n_records_per_day: Long, kafkaHandler: KafkaProducerHandler): Unit = {
